...
public class SQStats {
	private SQLite sqlite = Main.plugin.datenmanager.getSqlite();
	private final String table = "cookieclicker_players";

	public boolean inPlayers(String uuid) {

		try {
			ResultSet rs = sqlite.query("SELECT * FROM " + table + " WHERE UUID= '" + uuid + "'");
	...
	public void createPlayer(Player p) {
		...
		if (!(inPlayers(uuid))) {
			String status = "NONE";
			sqlite.update("INSERT INTO " + table
	...
	public void setName(String uuid, String name) {
		sqlite.update("UPDATE " + table + " SET NAME= '" + name + "' WHERE UUID= '" + uuid + "';");
	}

	public void setCookies(String uuid, Long cookies) {
		sqlite.update("UPDATE " + table + " SET COOKIES= '" + cookies + "' WHERE UUID= '" + uuid + "';");
	}

	public void setProclick(String uuid, Long proclick) {
		sqlite.update("UPDATE " + table + " SET PROCLICK= '" + proclick + "' WHERE UUID= '" + uuid + "';");
	}

	public void setDesign(String uuid, Long design) {
		sqlite.update("UPDATE " + table + " SET DESIGN= '" + design + "' WHERE UUID= '" + uuid + "';");
	}

	public void setClickerclicks(String uuid, Long clicks) {
		sqlite.update("UPDATE " + table + " SET CLICKERCLICKS= '" + clicks + "' WHERE UUID= '" + uuid + "';");
	}

	public void setMilk(String uuid, Long milk) {
		sqlite.update("UPDATE " + table + " SET MILK= '" + milk + "' WHERE UUID= '" + uuid + "';");
	}

	public void setPreis(String uuid, Integer zahl, Long preis) {
		if (zahl <= 0) {
			sqlite.update("UPDATE " + table + " SET PREIS= '" + preis + "' WHERE UUID= '" + uuid + "';");
		} else {
			sqlite.update("UPDATE " + table + " SET PREIS" + zahl + "= '" + preis + "' WHERE UUID= '" + uuid + "';");
		}
	}

	public void setBooster(String uuid, Integer zahl, String date) {
		if (zahl <= 0) {
			sqlite.update("UPDATE " + table + " SET BOOSTER= '" + date + "' WHERE UUID= '" + uuid + "';");
		} else {
			sqlite.update("UPDATE " + table + " SET BOOSTER" + zahl + "= '" + date + "' WHERE UUID= '" + uuid + "';");
		}
	}

	public ResultSet OrderBy(String type, int limit) {
		return sqlite.query("SELECT UUID FROM " + table + " ORDER BY " + type.toUpperCase() + " DESC LIMIT " + limit);
	}

	public ResultSet OrderBy(String type) {
		return sqlite.query("SELECT UUID FROM " + table + " ORDER BY " + type.toUpperCase() + " DESC");
	}
}
